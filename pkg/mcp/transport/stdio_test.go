...
func TestStdioDoubleConnect(t *testing.T) {
	...
	// Second Connect must fail, not leak a subprocess.
	if err := s.Connect(ctx); err == nil {
		t.Error("second Connect should return error")
	}
}

func TestStdioSubprocessOutlivesContext(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("skipping on windows (no cat)")
	}

	// Use a short-lived context for Connect. The subprocess must NOT be
	...
	// Wait for the Connect context to expire.
	<-ctx.Done()

	// The subprocess should still be alive. Send and receive a message.
	bgCtx, bgCancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer bgCancel()

	msg := json.RawMessage(`{"jsonrpc":"2.0","id":99,"method":"alive"}`)
	if err := s.Send(bgCtx, msg); err != nil {
		t.Fatalf("Send after context expiry: %v", err)
	}
	...
	case <-bgCtx.Done():
		t.Fatal("subprocess died after Connect context expired")
	}
}
